...
const initVault = async (a) => {
  ...
  iVault.undelegateAndClaimVault = async function (amount) {
  ...
  describe(`Inception pool V2 ${a.assetName}`, function () {
    ...
    describe("iVault setters", function () {
      ...
      it("setOperator(): only owner can", async function () {
        ...
        expect(events.length).to.be.eq(1);
        expect(events[0].args["prevValue"].toLowerCase()).to.be.eq(iVaultOperator.address.toLowerCase());
      ...
      it("addELOperator(): only owner can", async function () {
        ...
        expect(events.length).to.be.eq(1);
        expect(events[0].args["newELOperator"].toLowerCase()).to.be.eq(newELOperator.toLowerCase());
      });
      ...
      it("setDelegationManager(): only owner can", async function () {
        ...
        expect(events.length).to.be.eq(1);
        expect(events[0].args["prevValue"].toLowerCase()).to.be.eq(a.delegationManager.toLowerCase());
...
/*
      ...
      it("Stake more into strategy after delegate", async function() {
      ...
      it("depositAssetIntoStrategyFromVault: reverts when amount greater than free capacity", async function() {
        ...
        const amount = await iVault.totalAmountToWithdraw();
        await iVault.undelegateAndClaimVault(amount);

        await iVault.connect(staker).deposit(randomBI(18), staker.address);
        const totalAssets = await iVault.totalAssets();
        await iVault.connect(iVaultOperator).depositAssetIntoStrategyFromVault(totalAssets);
      })
    })
