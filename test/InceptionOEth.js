...
const initVault = async () => {
  ...
  console.log(`- stETH`);
  const stEthFactory = await hre.ethers.getContractFactory("stETH");
  stEth = stEthFactory.attach(stETHAddress);

  console.log(`- LidoMockPool`);
  const stEthPoolFactory = await hre.ethers.getContractFactory("LidoMockPool");
  stEthPool = stEthPoolFactory.attach(stETHAddress);

  // OEth
  const assetFactory = await hre.ethers.getContractFactory("OEth");
  const asset = assetFactory.attach(assetAddress);

  // VaultCore
  const assetPoolFactory = await hre.ethers.getContractFactory("VaultCore");
  ...
  await iToken.initialize("TEST InceptionLRT Token", "tINt");

  // 2. Impersonate operator
  const operator = await impersonateAccount(operatorAddress, toWei("1"));

  // 3. Inception vault
  const InceptionVaultFactory = await hre.ethers.getContractFactory("InoEthVault");
  const iVault = await InceptionVaultFactory.deploy();
  console.log(operatorAddress, strategyManagerAddress, iToken.address, assetStrategyAddress);
  await iVault.initialize(operatorAddress, strategyManagerAddress, iToken.address, assetStrategyAddress);
...
async function impersonateStaker(address, iVault, asset, assetPool) {
  ...
  await stEthPool.connect(staker).submit("0x0000000000000000000000000000000000000000", { value: toWei(20) });
  const balanceOfStEth = (await stEth.balanceOf(staker.address)).toString();
  await stEth.connect(staker).approve(assetPool.address, balanceOfStEth);

  await assetPool.connect(staker).mint(stETHAddress, balanceOfStEth, toBN(balanceOfStEth).sub(e18).toString());
  const balanceAfter = (await asset.balanceOf(staker.address)).toString();
...
describe("Inception ankrETH", function () {
  ...
  describe("Base flow", function () {
    ...
    it("Update asset ratio", async function () {
      await updateTheStrategyRatio(assetStrategyAddress, e18, staker);
