...
const strategyManagerAddress = "0x779d1b5315df083e3F9E94cB495983500bA8E907",
  stEthStrategyAddress = "0xB613E78E2068d7489bb66419fB1cfa11275d14da",
...
const initVault = async () => {
  ...
  // rETH
  console.log(`- stETH`);
  const assetFactory = await hre.ethers.getContractFactory("stETH");
  const asset = assetFactory.attach(stETHAddress);
  // rPool
  console.log(`- LidoMockPool`);
  const assetPoolFactory = await hre.ethers.getContractFactory("LidoMockPool");
  const assetPool = assetPoolFactory.attach(stETHAddress);
  ...
  // 3. Inception vault
  console.log(`- InstEthVault`);
  const InceptionVaultFactory = await hre.ethers.getContractFactory("InstEthVault");
  const iVault = await InceptionVaultFactory.deploy();
  /*
    In case of fail with: Error: VM Exception while processing transaction: reverted with reason string 'Initializable: contract is already initialized'
    Comment the 'constructor()' function in InrEthVault and InstEthVault and recompile
 */
  await iVault.initialize(operator.address, strategyManagerAddress, iToken.address, stEthStrategyAddress);
...
describe("Inception pool V3 stEth", function () {
  ...
  describe("Base flow", function () {
    ...
    it("Deposit", async function () {
      const amount = toBN("5893292334268593703");
    ...
    it("Update asset ratio", async function () {
      await updateTheStrategyRatio(stEthStrategyAddress, e18, staker);
  ...
  describe("withdraw: user can unstake", function () {
    ...
    before(async function () {
      ...
      await updateTheStrategyRatio(stEthStrategyAddress, e18, staker2);
  ...
  describe("withdrawFromEL: operator can request assets back from EL", function () {
    ...
    it("Staker redeems withdrawals", async function () {
      ...
      expect(stakerBalanceAfter.sub(stakerBalanceBefore)).to.be.closeTo(stakerUnstakeAmountAssetValue, 6);
      console.log(`Ratio: ${await iVault.ratio()}`);
  ...
  describe("redeem: to many addresses", function () {
    ...
    for (let j = 0; j < count; j++) {
      ...
      it(`${j} Deposit extra from iVault`, async function () {
        ...
        expect(totalDepositedAfter).to.be.closeTo(totalDepositedBefore, 2);
  ...
  describe("iToken ratio depends on the ratio of strategies", function () {
    ...
    testData.forEach(function (test) {
      it(`Transfer ${test.amount} to strategy`, async function () {
        ...
        await updateTheStrategyRatio(stEthStrategyAddress, test.amount, staker2);
