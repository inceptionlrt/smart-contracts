...
interface IFraxFerry {
    ...
    function getBatchAmount(uint256 start, uint256 end)
        external
        view
        returns (uint256 totalAmount);

    function getBatchData(uint256 start, uint256 end)
        external
        view
        returns (BatchData memory data);
    ...
    function setMinWaitPeriods(
        uint256 _MIN_WAIT_PERIOD_ADD,
        uint256 _MIN_WAIT_PERIOD_EXECUTE
    ) external;
