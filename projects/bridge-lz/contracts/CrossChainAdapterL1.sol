...
contract CrossChainAdapterL1 is Initializable, ICrossChainAdapterL1, OwnableUpgradeable {
    ...
    function initialize(
        ...
        require(
            _crosschainBridge != address(0) && _rebalancer != address(0) && _transactionStorage != address(0),
    ...
    function sendEthToL2(uint256 _chainId) external payable override onlyRebalancer {
        ...
        try bridge.quote(_chainId, "", "", false) returns (uint256 fee) {
            ...
            ICrossChainBridge(crosschainBridge).sendCrosschain{ value: totalEthSent }(_chainId, "", "");
        } catch Error(string memory reason) {
            emit QuoteError(_chainId, reason);
            ICrossChainBridge(crosschainBridge).sendCrosschain{ value: msg.value }(_chainId, "", "");
    ...
    function handleCrossChainData(uint256 _chainId, bytes calldata _payload) public override onlyCrosschainBridge {
        ...
        ITransactionStorage(transactionStorage).handleL2Info(_chainId, timestamp, balance, totalSupply);
    }
