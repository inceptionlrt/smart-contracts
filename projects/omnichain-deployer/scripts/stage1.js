...
const deployVaultAndToken = async (operatorAddress, strategyAddress, inceptionLibAddress, ratioFeedAddress) => {
    // 1. Inception token
    ...
    const feed = ethers.getContractAt("IInceptionRatioFeed", ratioFeedAddress); //"0x90D5a4860e087462F8eE15B52D9b1914BdC977B5");
  ...
  const deployERC20Rebalancer = async (
    ...
    // TODO set rebalancer as target receiver in L1 adapter
...
const main = async () => {
  // Stage 1: this deploys and configures L1 contracts that don't depend on knowing L2 addresses
    ...
    if (!obj.iTokenAddressL1 || !obj.iVaultAddressL1) {
        [obj.iTokenAddressL1, obj.iVaultAddressL1] = await deployVaultAndToken(await deployer.getAddress(), obj.strategyAddress, obj.inceptionLibAddress, obj.ratioFeedAddressL1);
    }
    await saveState(obj);
    if (!obj.rebalancer) {
