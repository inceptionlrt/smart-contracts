...
async function main() {
    ...
    const iovAddress = await updatedInceptionOmniVault.getAddress();

    // Display ETH balance of the updatedInceptionOmniVault at the start
    const initialBalanceIOV = await ethers.provider.getBalance(iovAddress);
    console.log(`Initial ETH balance of updatedInceptionOmniVault: ${ethers.formatUnits(initialBalanceIOV, "ether")} ETH`);

    //------------- CALLING THE SMART CONTRACT --------------------------------//

    console.log("Calling quoteSendEthCrossChain...");
    const fees = await updatedInceptionOmniVault.quoteSendEthCrossChain(17000n, options);
    ...
    const tx = await updatedInceptionOmniVault.sendEthCrossChain(17000n, options, { value: fees });
    console.log(`Transaction sent: ${tx.hash}`);

    // Display ETH balance of the updatedInceptionOmniVault at the end
    const finalBalanceIOV = await ethers.provider.getBalance(iovAddress);
    console.log(`Final ETH balance of updatedInceptionOmniVault: ${ethers.formatUnits(finalBalanceIOV, "ether")} ETH`);
}
