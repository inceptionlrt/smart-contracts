...
contract InceptionOmniVault is InceptionOmniAssetsHandler {
  ...
  function __InceptionOmniVault_init(
    ...
    if (_inceptionToken == address(0) || _operator == address(0)) revert NullParams();
   ...
   */
  function deposit(address receiver) external payable nonReentrant whenNotPaused returns (uint256) {
   ...
   */
  function _deposit(uint256 amount, address sender, address receiver) internal returns (uint256) {
    ...
    uint256 iShares = Convert.multiplyAndDivideFloor(amount + depositBonus, currentRatio, 1e18);
   ...
   */
  function flashWithdraw(uint256 iShares, address receiver) external whenNotPaused nonReentrant {
    __beforeWithdraw(receiver, iShares);

    address claimer = msg.sender;
    uint256 currentRatio = ratio();
    uint256 amount = Convert.multiplyAndDivideFloor(iShares, 1e18, currentRatio);
    uint256 capacity = getFlashCapacity();
    if (amount < minAmount) revert LowerMinAmount(minAmount);
   ...
   */
  function sendAssetsInfoToL1(bytes memory _options) external payable onlyOwnerOrOperator {
   ...
   */
  function quoteSendAssetsInfoToL1(bytes memory _options) external view returns (uint256 fees) {
   ...
   * @param _chaindId chain ID of the network to simulate sending ETH to
   * @param _options encoded params for cross-chain message. Includes `SEND_VALUE` which is substracted from the end result
   */
  function quoteSendEthCrossChain(uint256 _chaindId, bytes calldata _options) external view returns (uint256) {
    require(address(crossChainAdapter) != address(0), CrossChainAdapterNotSet());

    uint256 start = 16;
    uint256 end = start + 16;
    uint256 value = uint256(uint128(bytes16(_options[start:end])));
   ...
   * @return bonus Calculated bonus.
   */
  function _calculateDepositBonus(uint256 amount, uint256 capacity) internal view returns (uint256 bonus) {
  ...
  function getFreeBalance() public view returns (uint256 total) {
    return getFlashCapacity() < targetCapacity ? 0 : getFlashCapacity() - targetCapacity;
  }
  ...
  function convertToAssets(uint256 iShares) public view returns (uint256 assets) {
  ...
  function setDepositBonusParams(
    ...
    if (newMaxBonusRate > MAX_PERCENT) revert ParameterExceedsLimits(newMaxBonusRate);
    if (newOptimalBonusRate > MAX_PERCENT) revert ParameterExceedsLimits(newOptimalBonusRate);
    if (newDepositUtilizationKink > MAX_PERCENT) revert ParameterExceedsLimits(newDepositUtilizationKink);

    maxBonusRate = newMaxBonusRate;
    optimalBonusRate = newOptimalBonusRate;
    depositUtilizationKink = newDepositUtilizationKink;

    emit DepositBonusParamsChanged(newMaxBonusRate, newOptimalBonusRate, newDepositUtilizationKink);
  }
  ...
  function setFlashWithdrawFeeParams(
    ...
    if (newMaxFlashFeeRate > MAX_PERCENT) revert ParameterExceedsLimits(newMaxFlashFeeRate);
    if (newOptimalWithdrawalRate > MAX_PERCENT) revert ParameterExceedsLimits(newOptimalWithdrawalRate);
    if (newWithdrawUtilizationKink > MAX_PERCENT) revert ParameterExceedsLimits(newWithdrawUtilizationKink);
    ...
    emit WithdrawFeeParamsChanged(newMaxFlashFeeRate, newOptimalWithdrawalRate, newWithdrawUtilizationKink);
  }

  function setProtocolFee(uint64 newProtocolFee) external onlyOwner {
    if (newProtocolFee >= MAX_PERCENT) revert ParameterExceedsLimits(newProtocolFee);
  ...
  function setCrossChainAdapter(address payable newCrossChainAdapter) external onlyOwner {
