...
const [
  mellowWrapperAddress2,
  mellowVaultAddress2,
  mellowBondStrategyAddress2,
  mellowVaultOperatorAddress2,
  stEthAddress2,
  wstEthAddress2,
] = [
...
const initVault = async (a) => {
  ...
  const mellowVault = await ethers.getContractAt(
    "IMellowVault",
    mellowVaultAddress,
  );
  ...
  const mellowVault2 = await ethers.getContractAt(
    "IMellowVault",
    mellowVaultAddress2,
  );
  ...
  const mellowRestakerFactory =
    await ethers.getContractFactory("IMellowRestaker");
   ...
   */
  const mellowVaultOperatorMock = await ethers.deployContract("OperatorMock", [
    mellowBondStrategyAddress,
  ]);
  ...
  const mellowVaultOperator = await ethers.getContractAt(
    "OperatorMock",
    mellowVaultOperatorAddress,
  );
  iVault.withdrawFromMellowAndClaim = async function (amount) {
    await this.connect(iVaultOperator).undelegateFrom(
      mellowRestaker.address,
      amount,
    );
  ...
  const mellowVaultOperatorMock2 = await ethers.deployContract("OperatorMock", [
    mellowBondStrategyAddress2,
  ]);
  mellowVaultOperatorMock2.address =
    await mellowVaultOperatorMock2.getAddress();
  await network.provider.send("hardhat_setCode", [
    mellowVaultOperatorAddress2,
  ...
  const mellowVaultOperator2 = await ethers.getContractAt(
    "OperatorMock",
    mellowVaultOperatorAddress2,
  );
  iVault.withdrawFromMellowAndClaim = async function (amount) {
      ...
      await network.provider.send("hardhat_reset", [
        {
          forking: {
            ...
            blockNumber: a.blockNumber
              ? a.blockNumber
              : network.config.forking.blockNumber,
          },
        },
      ]);
    ...
    describe("Base flow no flash", function () {
      ...
      it("User can deposit to iVault", async function () {
        ...
        const tx = await iVault
          .connect(staker)
          .deposit(totalDeposited, staker.address);
      ...
      if (a.assetName === "stETH") {
        it("DelegateAuto to Mellow", async function () {
          ...
          await mellowRestaker.changeAllocation(
            mellowVault2.address,
            toWei(74),
          );
          await iVault.connect(iVaultOperator).delegateAuto();
          delegatedMellow += amount;

          const mellowBalance = await mellowVault.balanceOf(
            mellowRestaker.address,
          );
          const mellowBalance2 = await mellowVault2.balanceOf(
            mellowRestaker.address,
          );
          ...
          expect(totalDelegatedAfter).to.be.closeTo(
            delegatedMellow - 4n,
            transactErr,
          );
          expect(delegatedTo).to.be.closeTo(
            (totalDeposited * 25n) / 100n - 2n,
            transactErr,
          );
          expect(delegatedTo2).to.be.closeTo(
            (totalDeposited * 74n) / 100n - 2n,
            transactErr,
          );
          ...
          expect(await calculateRatio(iVault, iToken)).to.be.closeTo(
            e18,
            ratioErr,
          );
        });
      ...
      if (a.assetName === "stETH") {
        it("Add rewards to Mellow protocol and estimate ratio", async function () {
          ...
          console.log(
            `Delegated to before:\t${totalDelegatedToBefore.format()}`,
          );
          ...
          const wstEthErc20 = await ethers.getContractAt(
            "IERC20",
            wstEthAddress,
          );
          ...
          await wstEthErc20
            .connect(staker3)
            .transfer(mellowVaultAddress, wrtEthAmount);
          ...
          console.log(
            `Delegated to after:\t\t${totalDelegatedToAfter.format()}`,
          );
          console.log(`mellow rewards:\t\t\t${rewardsMellow.format()}`);
          await ratioFeed.updateRatioBatch([iToken.address], [ratioAfter]);
          expect(totalDelegatedAfter - totalDelegatedBefore).to.be.eq(
            totalDelegatedToAfter - totalDelegatedToBefore,
          );
        });
      }
      ...
      it("Estimate the amount that user can withdraw", async function () {
        ...
        expect(assetValue).closeTo(
          totalDeposited + rewardsEL + rewardsMellow,
          transactErr * 10n,
        );
      });
      ...
      it("User can withdraw all", async function () {
        ...
        const tx = await iVault
          .connect(staker)
          .withdraw(shares, staker2.address);
        const receipt = await tx.wait();
      ...
      if (a.assetName === "stETH") {
        it("Withdraw from Mellow", async function () {
          ...
          console.log(
            `Total assets before:\t\t\t${totalAssetsBefore.format()}`,
          );
          ...
          await iVault
            ...
            .undelegateFrom(mellowVault.address, amount);
          await iVault
            .connect(iVaultOperator)
            .undelegateFrom(mellowVault2.address, amount2);
          ...
          const totalDepositedAfter = await iVault.getTotalDeposited();
          const pendingWithdrawalsMellowAfter =
            await iVault.getPendingWithdrawalAmountFromMellow();

          console.log(`Total assets after:\t\t${totalAssetsAfter.format()}`);
          console.log(
            `Total delegated after:\t${totalDelegatedAfter.format()}`,
          );
          console.log(
            `Total deposited after:\t${totalDepositedAfter.format()}`,
          );
          console.log(
            `Pending from Mellow:\t${pendingWithdrawalsMellowAfter.format()}`,
          );
          ...
          expect(totalDepositedAfter).to.be.closeTo(
            ...
            transactErr,
          ); //Total deposited amount did not change
          expect(pendingWithdrawalsMellowAfter).to.be.closeTo(
            amount + amount2 - 4n,
            transactErr,
          ); // Diff of 4wei
        });
      ...
      if (a.assetName === "stETH") {
        it("Process request from the Mellow", async function () {
          const totalDepositedBefore = await iVault.getTotalDeposited();
          const pendingWithdrawalsMellowBefore =
            await iVault.getPendingWithdrawalAmountFromMellow();
          console.log(
            `Total deposited before:\t\t\t${totalDepositedBefore.format()}`,
          );
          console.log(
            `Pending from Mellow before:\t${pendingWithdrawalsMellowBefore.format()}`,
          );

          await mellowVaultOperator.processWithdrawals([
            mellowRestaker.address,
          ]);
          await mellowVaultOperator2.processWithdrawals([
            mellowRestaker.address,
          ]);
      ...
      it("Staker is able to redeem", async function () {
        ...
        console.log(
          `Staker2 pending withdrawals: ${await iVault.getPendingWithdrawalOf(staker2.address)}`,
        );
