...
const assets = [
  {
    assetName: "LBTC",
    assetAddress: "0x8236a87084f8B84306f72007F36F2618A5634494",
    ...
    impersonateStaker: async function (staker, iVault) {
      const assetAddress = "0x8236a87084f8B84306f72007F36F2618A5634494";
      const donorAddress = "0x3752edF4Ca393a1497F7C50298FbBC4F3EF40368";
      const amount = 100 * 1e8;

      const donor = await impersonateWithEth(donorAddress, toWei(1));
      const lbtc = await ethers.getContractAt("LBTC", assetAddress);

      await lbtc.connect(donor).transfer(staker.address, amount);
      await lbtc.connect(staker).approve(await iVault.getAddress(), amount);
    ...
    addRewardsMellowVault: async function (amount, mellowVault) {
      ...
      const donor = await impersonateWithEth(donorAddress, toWei(1));
      const lbtc = await ethers.getContractAt("LBTC", assetAddress);
      await lbtc.connect(donor).transfer(mellowVault, amount);
    },
  },
  // {
  //   assetName: "stETH",
...
const symbioticVaults = [
  {
    name: "MEV LBTC Vault",
    vaultAddress: "0xdC47953c816531a8CA9E1D461AB53687d48EEA26",
  },
];
...
const initVault = async a => {
  ...
  await iVault.setSymbioticRestaker(symbioticRestaker.address);
  ...
  return [iToken, iVault, ratioFeed, asset, iVaultOperator, mellowRestaker, symbioticRestaker, iLibrary];
};

assets.forEach(function (a) {
  describe(`Inception Symbiotic Vault ${a.assetName}`, function () {
    this.timeout(150000);
    let iToken, iVault, ratioFeed, asset, mellowRestaker, symbioticRestaker, iLibrary;
    ...
    before(async function () {
      ...
      [iToken, iVault, ratioFeed, asset, iVaultOperator, mellowRestaker, symbioticRestaker, iLibrary] =
        await initVault(a);
      ratioErr = a.ratioErr;
    ...
    describe("Symbiotic Native | Base flow no flash", function () {
      ...
      it("Delegate to symbioticVault#1", async function () {
        ...
        const symbioticBalance = await symbioticVaults[0].vault.activeBalanceOf(symbioticRestaker.address);
        // const symbioticBalance2 = await symbioticVaults[1].vault.activeBalanceOf(symbioticRestaker.address);
      ...
      //   await expect(symbioticRestaker.addVault(symbioticVaults[1].vaultAddress))
      //     .to.emit(symbioticRestaker, "VaultAdded")
      ...
      //   const totalAssetsBefore = await iVault.totalAssets();
      //
      //   await iVault.connect(iVaultOperator).delegateToSymbioticVault(symbioticVaults[1].vaultAddress, amount);
      //   delegatedSymbiotic += amount;
      //
      //   const symbioticBalance = await symbioticVaults[0].vault.activeBalanceOf(symbioticRestaker.address);
      //   const symbioticBalance2 = await symbioticVaults[1].vault.activeBalanceOf(symbioticRestaker.address);
      ...
      it("Undelegate from Symbiotic", async function () {
        ...
        const totalAssetsAfter = await iVault.totalAssets();
        const totalDelegatedAfter = await iVault.getTotalDelegated();
        const totalDelegatedTo = await symbioticRestaker.getDeposited(symbioticVaults[0].vaultAddress);
        // const totalDelegatedTo2 = await symbioticRestaker.getDeposited(symbioticVaults[1].vaultAddress);
        const totalDepositedAfter = await iVault.getTotalDeposited();
        const pendingWithdrawalsSymbioticAfter = await symbioticRestaker.pendingWithdrawalAmount();
        ...
        expect(pendingWithdrawalsSymbioticAfter).to.be.closeTo(amount, transactErr * 2n);
      });

      it("Process request to transfers pending funds to symbioticRestaker", async function () {
        ...
        let nextEpochStart2, epochDuration2;

        const maxNextEpochStart = nextEpochStart1;
        const maxEpochDuration = epochDuration1;
        ...
        // const totalDepositedBefore = await iVault.getTotalDeposited();
        // const pendingWithdrawalsMellowBefore = await symbioticRestaker.pendingWithdrawalAmount();
        // const restakerBalanceBefore = await asset.balanceOf(symbioticRestaker.address);
        // console.log(`Total deposited before:\t\t\t${totalDepositedBefore.format()}`);
      ...
      it("Claim Symbiotic withdrawal transfer funds from Symbiotic to the vault", async function () {
        const pendingWithdrawalsSymbiotic = await symbioticRestaker.pendingWithdrawalAmount();
        const totalAssetsBefore = await iVault.totalAssets();
        const restakerBalanceBefore = await asset.balanceOf(symbioticRestaker.address);

        await iVault
          .connect(iVaultOperator)
          .claimCompletedWithdrawalsSymbiotic(
            symbioticVaults[0].vaultAddress,
            (await symbioticVaults[0].vault.currentEpoch()) - 1n,
          );

        // await iVault
        //   .connect(iVaultOperator)
        //   .claimCompletedWithdrawalsSymbiotic(
        //     symbioticVaults[1].vaultAddress,
        //     (await symbioticVaults[1].vault.currentEpoch()) - 1n,
        //   );

        const totalAssetsAfter = await iVault.totalAssets();
        const restakerBalanceAfter = await asset.balanceOf(symbioticRestaker.address);

        expect(totalAssetsAfter - totalAssetsBefore).to.be.closeTo(pendingWithdrawalsSymbiotic, transactErr);
        expect(restakerBalanceBefore).to.be.closeTo(restakerBalanceAfter, transactErr);
      });
    ...
    describe("Base flow with flash withdraw", function () {
      ...
      it("Delegate freeBalance", async function () {
        ...
        console.log("free balance", amount);

        await iVault.connect(iVaultOperator).delegateToSymbioticVault(symbioticVaults[0].vaultAddress, amount);

        console.log(await symbioticRestaker.getTotalDeposited());

        const delegatedTotal = await iVault.getTotalDelegated();
        const delegatedTo = await iVault.getDelegatedToSymbiotic(symbioticVaults[0].vaultAddress);
      ...
      it("Undelegate from Mellow", async function () {
        ...
        const amount = await iVault.getDelegatedToSymbiotic(symbioticVaults[0].vaultAddress);
        await iVault.connect(iVaultOperator).undelegateFromSymbiotic(symbioticVaults[0].vaultAddress, amount);

        const totalAssetsAfter = await iVault.totalAssets();
        const totalDelegatedAfter = await iVault.getTotalDelegated();
        const totalDelegatedTo = await iVault.getDelegatedToSymbiotic(symbioticVaults[0].vaultAddress);
        const totalDepositedAfter = await iVault.getTotalDeposited();
        const pendingWithdrawalsMellowAfter = await symbioticRestaker.pendingWithdrawalAmount();
    ...
    */
      ...
      args.forEach(function(arg) {
        ...
        amounts.forEach(function(amount) {
          it(`calculateDepositBonus for ${amount.name}`, async function() {
            ...
            console.log("amount ", await amount.amount());
    ...
    describe("Withdraw fee params setter and calculation", function() {
      ...
      const amounts = [
        ...
        },
        {
          ...
          flashCapacity: targetCapacity => targetCapacity,
          amount: async () => (10n ** 8n * 75n) / 100n + 1n,
        },
        {
      ...
      it("calculateFlashWithdrawFee reverts when capacity is not sufficient", async function() {
        ...
        await iVault.setWithdrawMinAmount(1e4);
        await iVault.setFlashMinAmount(1e4);
        await iVault.connect(staker, staker).deposit(randomBI(9), staker.address);
    ...
    describe("Deposit: user can restake asset", function () {
      ...
      before(async function () {
        ...
        console.log(" ######################## init Ratio: ", await calculateRatio(iVault, iToken));
        console.log("  ====== === === === === === === === === === === === === === ===  ");
      ...
      const args = [
        // {
        //   amount: async () => 479807293932n,
        //   receiver: () => staker.address,
        // },
        {
          amount: async () => 99999999n,
          receiver: () => ethers.Wallet.createRandom().address,
        },
        {
          amount: async () => 88888888n,
          receiver: () => staker.address,
        },
        {
          amount: async () => 77777777n,
          receiver: () => staker.address,
        },
        {
          amount: async () => 66666666n,
          receiver: () => staker.address,
        },
        {
          amount: async () => 55555555n,
          receiver: () => staker.address,
        },
        {
          amount: async () => 44444444n,
          receiver: () => staker.address,
        },
        {
          amount: async () => 33333333n,
          receiver: () => staker.address,
        },
        {
          amount: async () => 22222222n,
          receiver: () => staker.address,
        },
        {
          amount: async () => 11111111n,
          receiver: () => staker.address,
        },
        {
          amount: async () => (await iVault.convertToAssets(await iVault.withdrawMinAmount())) + 1n,
    ...
    describe.skip("Deposit with bonus for replenish", function () {
      const states = [
        {
    ...
    describe("Delegate to mellow vault", function() {
      ...
      beforeEach(async function() {
        ...
        await iVault.connect(iVaultOperator).delegateToMellowVault(mellowVaults[0].vaultAddress, firstDeposit, 1296000);
      ...
      args.forEach(function(arg) {
        it(`Deposit and delegate ${arg.name} many times`, async function() {
          ...
          for (let i = 0; i < count; i++) {
            ...
            await iVault
              .connect(iVaultOperator)
              .delegateToMellowVault(mellowVaults[0].vaultAddress, delegated, 1296000);
      ...
      args2.forEach(function(arg) {
        it(`Deposit many times and delegate once ${arg.name}`, async function() {
          ...
          await iVault
            .connect(iVaultOperator)
            .delegateToMellowVault(mellowVaults[0].vaultAddress, totalDelegated, 1296000);
      ...
      args3.forEach(function(arg) {
        it(`Delegate many times ${arg.name}`, async function() {
          ...
          for (let i = 0; i < arg.count; i++) {
            ...
            await expect(iVault.connect(iVaultOperator).delegateToMellowVault(mVault, amount, 1296000))
              .to.emit(iVault, "DelegatedTo")
      ...
      invalidArgs.forEach(function(arg) {
        it(`delegateToMellowVault reverts when ${arg.name}`, async function() {
          ...
          if (arg.customError) {
            await expect(
              iVault.connect(operator).delegateToMellowVault(mVault, delegateAmount, 1296000),
            ).to.be.revertedWithCustomError(arg.source(), arg.customError);
          } else {
            await expect(iVault.connect(operator).delegateToMellowVault(mVault, delegateAmount, 1296000)).to.be
              .reverted;
          }
        });
      });
    ...
    */
    ...
    describe("Withdraw: user can unstake", function () {
      ...
      before(async function () {
        ...
        await iVault.setDepositMinAmount(1e2);
        await iVault.setWithdrawMinAmount(1e2);
        await iVault.setFlashMinAmount(1e2);

        console.log("curr ratio", await iVault.ratio());

        console.log(await iVault.depositMinAmount());
      ...
      const testData = [
        {
          name: "random e18",
          amount: async shares => 72432598n,
          receiver: () => staker.address,
        },
        {
          name: "99999999",
          amount: async shares => 99999999n,
          receiver: () => staker2.address,
        },
        {
          name: "8888888",
          amount: async shares => 8888888n,
          receiver: () => staker2.address,
        },
        // {
      ...
      testData.forEach(function (test) {
        it(`Withdraw ${test.name}`, async function () {
          ...
          console.log("------ DEBUG ------");
          console.log(await iVault.getTotalDeposited(), totalDeposited);
    ...
    describe("Withdraw: negative cases", function () {
      ...
      it("Withdraw small amount many times", async function () {
        ...
        await iVault.connect(staker).withdraw(1e8, staker.address);
      ...
      it("Reverts: withdraw when iVault is paused", async function () {
        await iVault.pause();
        await expect(iVault.connect(staker).withdraw(1e5, staker.address)).to.be.revertedWith("Pausable: paused");
        await iVault.unpause();
      });

      it("Reverts: withdraw when targetCapacity is not set", async function () {
        await snapshot.restore();
        await expect(iVault.connect(staker).withdraw(1e5, staker.address)).to.be.revertedWithCustomError(
          iVault,
          "InceptionOnPause",
        );
      });
    ...
    describe.skip("Flash withdraw with fee", function () {
      const targetCapacityPercent = e18;
      const targetCapacity = e18;
      let deposited = 0n;
      beforeEach(async function () {
        ...
        const freeBalance = await iVault.getFreeBalance();
        await iVault.connect(iVaultOperator).delegateToMellowVault(mellowVaults[0].vaultAddress, freeBalance, 1296000);
    ...
    describe.skip("Mellow vaults management", function () {
      beforeEach(async function () {
        await snapshot.restore();
        await iVault.setTargetFlashCapacity(1n);
        await iVault.connect(staker).deposit(e18, staker.address);
      ...
      it("changeMellowWrapper", async function () {
        ...
        const freeBalance = await iVault.getFreeBalance();
        await expect(iVault.connect(iVaultOperator).delegateToMellowVault(mellowVault, freeBalance, 1296000))
    ...
    describe.skip("undelegateFromMellow: request withdrawal from mellow vault", function () {
      ...
      it("Delegate to mellowVault#1", async function () {
        ...
        await iVault
          .connect(iVaultOperator)
          .delegateToMellowVault(mellowVaults[0].vaultAddress, vault1Delegated, 1296000);
      ...
      it("Add mellowVault#2 and delegate the rest", async function () {
        ...
        await iVault
          .connect(iVaultOperator)
          .delegateToMellowVault(mellowVaults[1].vaultAddress, vault2Delegated, 1296000);
      ...
      it("undelegateFromMellow from mellowVault#1 by operator", async function () {
        ...
        await expect(
          iVault.connect(iVaultOperator).undelegateFromMellow(mellowVaults[0].vaultAddress, assets1, 1296000),
        )
      ...
      it("undelegateFromMellow replaces pending withdraw from mellowVault#1", async function () {
        ...
        await expect(iVault.connect(iVaultOperator).undelegateFromMellow(mellowVaults[0].vaultAddress, amount, 1296000))
          .to.emit(iVault, "StartMellowWithdrawal")
      ...
      it("undelegateFromMellow all from mellowVault#2", async function () {
        ...
        await expect(
          iVault
            .connect(iVaultOperator)
            .undelegateFromMellow(mellowVaults[1].vaultAddress, vault2Delegated + 1000_000_000n, 1296000),
        )
          .to.emit(iVault, "StartMellowWithdrawal")
          .withArgs(mellowRestaker.address, a => {
      ...
      it("Can not claim when restaker balance is 0", async function () {
        await expect(iVault.connect(iVaultOperator).claimCompletedWithdrawalsMellow()).to.be.revertedWithCustomError(
          mellowRestaker,
          "ValueZero",
        );
      });
      ...
      it("Can not claim funds from mellowRestaker when iVault is paused", async function () {
        await iVault.pause();
        await expect(iVault.connect(iVaultOperator).claimCompletedWithdrawalsMellow()).to.be.revertedWith(
          "Pausable: paused",
        );
      });
    ...
    describe.skip("undelegateFromMellow: negative cases", function () {
      beforeEach(async function () {
        await snapshot.restore();
        await iVault.setTargetFlashCapacity(1n);
        await iVault.connect(staker).deposit(randomBI(19), staker.address);
      ...
      invalidArgs.forEach(function (arg) {
        it(`Reverts: when ${arg.name}`, async function () {
          ...
          if (arg.customError) {
            await expect(
              iVault.connect(arg.operator()).undelegateFromMellow(mellowVault, amount, 1296000),
            ).to.be.revertedWithCustomError(arg.source(), arg.customError);
          } else {
            await expect(
              iVault.connect(arg.operator()).undelegateFromMellow(mellowVault, amount, 1296000),
      ...
      it("Reverts: undelegate when iVault is paused", async function () {
        ...
        await iVault.pause();
        await expect(
          iVault.connect(iVaultOperator).undelegateFromMellow(mellowVaults[0].vaultAddress, amount, 1296000),
        ).to.be.revertedWith("Pausable: paused");
        await iVault.unpause();
      });
    ...
    //     await expect(
    //       mellowRestaker.connect(staker).withdrawEmergencyMellow(mellowVaults[0].vaultAddress, 1296000),
    //     ).to.revertedWithCustomError(mellowRestaker, "NotVaultOrTrusteeManager");
    //   });
    ...
    describe("Redeem: to the different addresses", function () {
      ...
      for (let j = 0; j < count; j++) {
        ...
        it(`${j} Withdraw from EL and update ratio`, async function () {
          const amount = await iVault.totalAmountToWithdraw();
          await iVault.connect(iVaultOperator).undelegateFromMellow(mellowVaults[0].vaultAddress, amount, 1296000);
