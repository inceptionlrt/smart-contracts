...
contract MockFraxferry {
    ...
    uint public immutable targetChain;
    ...
    uint public constant REDUCED_DECIMALS = 1e10;
    ...
    function embarkWithRecipient(uint amount, address recipient) public {
        ...
        if (fee_exempt_addrs[msg.sender]) fee = 0;
            ...
            fee = Math.min(
                Math.max(FEE_MIN, (amount * FEE_RATE) / 10000),
                FEE_MAX
            );
        }
        require(amount > fee, "Amount too low");
        require(
            amount / REDUCED_DECIMALS <= type(uint64).max,
            "Amount too high"
        );
