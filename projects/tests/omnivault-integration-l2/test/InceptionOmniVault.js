...
async function init(operator) {
  ...
  const omniVault = await upgrades.deployProxy(omniVaultFactory,
    ["Omnivault", operator.address, iToken.address, adapter],
...
describe("Inception omni vault", function() {
  ...
  before(async function () {
    [owner, operator, staker1, staker2, staker3] = await ethers.getSigners();
    [iToken, omniVault, ratioFeed] = await init(operator);
